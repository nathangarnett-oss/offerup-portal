...
import { mockJobAnalytics } from '@/lib/mock-data';
import { JOB_TYPES, JOB_CATEGORIES } from '@/lib/constants';
import { formatNumber, formatDateLong, cn } from '@/lib/utils';
import {
  ArrowLeft,
  Eye,
  MousePointerClick,
...
function formatPay(payType: string, payMin: number, payMax: number): string {
  if (payType === 'salary') {
    return `$${(payMin / 1000).toFixed(0)}K - $${(payMax / 1000).toFixed(0)}K/yr`;
  }
...
export default function JobPerformancePage() {
  ...
  const analytics = mockJobAnalytics[params.id as string];
  ...
  if (!job) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Link href="/portal/jobs" className="mt-4 text-offerup-green hover:underline">
          Back to My Jobs
        </Link>
      </div>
    );
  ...
  const metrics = [
    ...
    { label: 'Applications', value: (analytics?.summary.totalApplications ?? 0).toString(), icon: Users, color: 'text-green-600', bg: 'bg-green-50' },
    ...
    { label: 'Applicants', value: (job.applicationCount ?? 0).toString(), icon: Briefcase, color: 'text-indigo-600', bg: 'bg-indigo-50' },
  ];
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {job.companyName} &middot; Posted {formatDateLong(job.createdAt)}
            </p>
          </div>
        </div>
        <Link
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-600">
          <span className="inline-flex items-center gap-1.5">
            ...
            {typeLabel} &middot; {categoryLabel}
          </span>
          <span className="inline-flex items-center gap-1.5">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              <defs>
                ...
                <linearGradient id="colorJobClicks" x1="0" y1="0" x2="0" y2="1">
              ...
              />
              <Legend />
              <Area type="monotone" dataKey="views" stroke="#00AB80" fillOpacity={1} fill="url(#colorViews)" strokeWidth={2} />
              <Area type="monotone" dataKey="clicks" stroke="#6366F1" fillOpacity={1} fill="url(#colorJobClicks)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>

      {/* Daily Applications & Click-Through Rate */}
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-base font-semibold text-gray-800">Daily Applications</h2>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Bar dataKey="applications" fill="#00AB80" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
